...
//#define AT88_GET_MFandLOT_INFO _IO(AT88_IOCTL_MAGIC, 2)
#define AT88_GET_MFandLOT_INFO 18690
...
static int at88_open(struct inode *inode,struct file *filp)
{
	...
	//test for menufacture's and lot's code
	memset(ch, 0, 12);
	ucReturn = cm_ReadConfigZone(0x00, ch, 10);
	printk(KERN_ALERT "0x00readl(10)ucReturn:%#x, ch:", ucReturn);
	print_buf(ch, 10);//print menufacture and lot codes 16 + 64 bytes in total,  at88sc104 -> |0x3b 0xb2| + |0x11 0x0 0x10 0x80 0x0 0x1 0x10 0x10|
...
struct file_operations at88_fops = {
	.open			= at88_open,
...
static int __init davinci_at88_init(void)
{
	...
	p_at88_dev = kzalloc(sizeof(struct at88_dev_t), GFP_KERNEL);
	if(!p_at88_dev)
	{
		ret = -ENOMEM;
		printk(KERN_ERR "Error:kzalloc for p_at88_dev failed!\n");
		goto fail_p_at88_dev_kzalloc;
	}
	p_at88_dev->devno=MKDEV(CM_MAJOR,0);
	if (at88_major)
	{
		ret= register_chrdev_region(p_at88_dev->devno, 1, "stingy");
	}
	else
	{
		ret = alloc_chrdev_region(&p_at88_dev->devno, 0,1, "stingy");	
		at88_major = MAJOR(p_at88_dev->devno);
	} 
	if(ret<0)
	{
		printk(KERN_ERR "Error:chrdev_region for p_at88_dev failed!\n");
		ret = -EAGAIN;
		goto err_chrdev_region;
	}

	cdev_init(&p_at88_dev->cdev,&at88_fops);
	p_at88_dev->cdev.owner = THIS_MODULE;
	ret=cdev_add(&p_at88_dev->cdev, p_at88_dev->devno, 1);
	if(ret < 0)
	{
		goto fail_cdev_add;
	}
	at88_class =class_create(THIS_MODULE,"stingy");
	if (IS_ERR(at88_class))
	{
		ret = PTR_ERR(at88_class);
		printk("%s class_create failed!\n",__func__);
		goto class_err;
	}

	dev = device_create(at88_class,NULL, p_at88_dev->devno,NULL,"stingy%d",0); 
	if (IS_ERR(dev)) {
		ret = PTR_ERR(dev);
...
class_err:
	cdev_del(&p_at88_dev->cdev);
fail_cdev_add:
	unregister_chrdev_region(p_at88_dev->devno,  1);
err_chrdev_region:
	kfree(p_at88_dev);
fail_p_at88_dev_kzalloc:
	iounmap(regdir);
err_regdir_ioremap:
	iounmap(regdat);
err_regdat_ioremap:
...
{
	device_destroy(at88_class, p_at88_dev->devno);
	class_destroy(at88_class);
	cdev_del(&p_at88_dev->cdev);
	unregister_chrdev_region(p_at88_dev->devno,  1);
	kfree(p_at88_dev);
