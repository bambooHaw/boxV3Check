...
int main(int argc, char* argv[])
{
	...
	char result_buf[USR_ZONE_MAX_LENGTH] = {};
	...
	ret = ioctl(fd, AT88_SET_BLK, msg.usrZoneNum);

	if(_IOC_READ == _IOC_DIR(cmd))
	{
		bzero(msg.buf, sizeof(msg.buf));
		ret = read(fd, msg.buf, USR_ZONE_MAX_LENGTH);
		bzero(result_buf, sizeof(result_buf));
		memcpy(result_buf, msg.buf+msg.index, msg.length);
		//test
		printf("read:");
		show_buf(result_buf, msg.length);
	}else
	{
		bzero(result_buf, sizeof(result_buf));
		ret = read(fd, result_buf, USR_ZONE_MAX_LENGTH);
		memcpy(result_buf+msg.index, msg.buf, msg.length);
		ret = write(fd, result_buf, USR_ZONE_MAX_LENGTH);
	}
