...
int at88_status=AT88_STATUS_ERROR; // -1 Éè±¸´íÎó»òÎ´¼ì²âµ½  0 Éè±¸Î´³õÊ¼»¯ 1 Éè±¸³õÊ¼»¯£¬µ«ÊÇÃ»ÓÐÉÕfuse 2 Éè±¸Î´³õÊ¼»¯£¬µ«ÊÇfuseÒÑÉÕ 3 Éè±¸³õÊ¼»¯²¢ÇÒÉÕÁËfuse
...
static int at88_open(struct inode *inode,struct file *filp)
{
	...
	at88sc_private_t *info = kzalloc(sizeof(at88sc_private_t), GFP_KERNEL);
	if(!info){
		printk(KERN_ERR "malloc for at88sc_private failed!");
		filp->private_data = NULL;
		ret = -ENOMEM;
	}else{
		filp->private_data = info;
		info->contention = mutex_trylock(&usr_access_lock);
		if(!info->contention){
			printk(KERN_WARNING "at88_open failed! Device is busy now, please try again later!");
			ret = -EBUSY;
		}else{
#if 0
			//test20180402 @henry
			...
			ucReturn = cm_ReadConfigZone(0x00, buf, 32);
			printk(KERN_ALERT "\n mfandlots:");
			print_buf(buf, 32);
			
			memset(buf, 0, sizeof(buf));
			ucReturn = cm_ReadConfigZone(0x28, buf, 24);
			printk(KERN_ALERT "\n passkeyandpasszone:");
			print_buf(buf, 32);
...
{
	int ret = 0;
	at88sc_private_t *info = (at88sc_private_t*)(filp->private_data);
	if(!info){
		printk(KERN_WARNING "at88sc's private_data is emptyï¼");
		ret = -EINVAL;
	}else{
		if(info->contention){			
			//power off the chip
			cm_PowerOff();
			mutex_unlock(&usr_access_lock);
			ret = 0;
		}else{
			ret = -EAGAIN;
		}
...
long at88_ioctl(struct file *fd, unsigned int cmd, unsigned long arg)
{
	...
	at88sc_ioctl_arg_t* info = NULL;

	at88sc_private_t *inf = (at88sc_private_t*)(fd->private_data);
	if(!inf){
		return -EINVAL;
	}else{
		if(!inf->contention){			
	...
	switch(cmd)
	{
		...
		case AT88_GET_COMMON_INFO:
		{
			ret = 0;
			memset(buf, 0, sizeof(buf));
			info = (at88sc_ioctl_arg_t*)arg;
			if(!info)
			{
				ret = -EINVAL;
				break;
			}
			ucReturn = cm_ReadConfigZone(info->addr, buf, 8);
			//printk(KERN_ALERT "0x00readl(10)ucReturn:%#x, ch:", ucReturn);
			if(copy_to_user(info->buf, buf, 8) > 0){
				printk(KERN_ERR "copy to user get something wrong!\n");
				ret = -EAGAIN;
			}
			print_buf(buf, 8);
			print_buf(info->buf, 8);
			break;
		}
		default:
		{
